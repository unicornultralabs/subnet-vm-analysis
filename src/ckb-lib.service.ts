...
class LumosService {
  generateAccountFromPrivateKey(privKey: string): Account {
  ...
  utf8ToHex(utf8String: string): string {
  ...
  hexToUtf8(hexString: string): string {
  ...
  async readOnChainMessage(txHash: string, index = '0x0') {
    const { cell } = await rpc.getLiveCell({ txHash, index }, true);
    if (cell == null) {
      throw new Error('cell not found, please retry later');
    }
    const data = cell.data.content;
    const msg = this.hexToUtf8(data);
    return msg;
  }

  async buildMessageTx(
    onChainMemo: string,
    privateKey: string = '0x6109170b275a09ad54877b82f7d9930f88cab5717d484fb4741ae9d1dd078cd6',
  ): Promise<string> {
    let txSkeleton = helpers.TransactionSkeleton({});
    const fromAccount = this.generateAccountFromPrivateKey(privateKey);
    const onChainMemoHex: HexString = this.utf8ToHex(onChainMemo);
...
export default new LumosService();
